...
auto DefaultMode::get_train(std::vector<std::string>& vec) const
  ...
  int nwords;
  curs_set(1);
  mvwprintw(winptr, my / 2, (mx - 22) / 2, "%s", "Enter amount of words: ");
  echo();
  ...
  std::sample(vec.cbegin(), vec.cend(), std::back_inserter(samples), nwords,
  ...
  if (ss.str() != "") train.push_back(ss.str());
  return train;
}

DefaultMode::DefaultMode(Dictionary& dict) noexcept : Mode(), dict_(dict) {
  all_ = 0;
  correct_ = 0;
  winptr_ = window().get_window();
}

auto DefaultMode::preprocess() -> std::vector<std::string> {
  auto vec = dict_.store().at("default.txt");
  return get_train(vec);
}

auto DefaultMode::loop(std::string_view w) -> void {
  int my, mx;
  ...
  for (const auto& s : w) {
    ...
    if (s != rs)
      mvwaddch(winptr_, begy, begx, rs | A_BOLD | COLOR_PAIR(1));
    else
      ++correct_;
    ++all_;
    ++begx;
