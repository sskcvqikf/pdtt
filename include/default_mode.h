...
struct DefaultMode final : public Mode<DefaultMode> {
  ...
  auto preprocess() -> std::vector<std::string>;

  auto postprocess(std::chrono::duration<long, std::ratio<1, 1000000000>>)
      const noexcept -> Stats;

  auto loop(std::string_view s) -> void;

  auto get_dict() -> Dictionary;

 private:
  auto get_train(std::vector<std::string>&) const -> std::vector<std::string>;
